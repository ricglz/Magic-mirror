...
def correct_colours(im1, im2, landmarks1):
    im1_blur, im2_blur = get_im_blur(
        im1, im2, landmarks1, COLOUR_CORRECT_BLUR_FRAC
    )
...
def get_mask(size, dst_points, warped_src_face):
    h, w = size
    mask = mask_from_points((h, w), dst_points)
    mask_src = np.mean(warped_src_face, axis=2) > 0
    return np.asarray(mask * mask_src, dtype=np.uint8)

def correct_warped_face_color(warped_src_face, mask, dst_face, dst_points):
    warped_src_face = apply_mask(warped_src_face, mask)
    dst_face_masked = apply_mask(dst_face, mask)
    return correct_colours(dst_face_masked, warped_src_face, dst_points)

def get_2d_warped_mask(
    warped_src_face,
    dst_points,
    src_points,
    src_face,
...
def face_swap(src_face, dst_face, src_points, dst_points, dst_shape, dst_img):
    ...
    warped_src_face = warp_image_3d(
        src_face, src_points[:END], dst_points[:END], dst_size
    )

    ## Mask for blending
    mask = get_mask(dst_size, dst_points, warped_src_face)

    ## Correct color
    if CORRECT_COLOR:
        warped_src_face = correct_warped_face_color(
            warped_src_face, mask, dst_face, dst_points
        )

    ## 2D warp
    if WARP_2D:
        mask = get_2d_warped_mask(
